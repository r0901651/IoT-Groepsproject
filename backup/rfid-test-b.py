...
# Disable GPIO warnings and set mode
...
while not client.is_connected():
    ...
    time.sleep(interval)  # Add a delay after each attempt
...
def check_button():
    ...
    while True:
        ...
        print(f"Button state: {button_state}")  # Added debug information
        if button_state == GPIO.HIGH:
            time.sleep(0.2)  # Increased debounce delay
            button_state = GPIO.input(BUTTON_PIN)
            if button_state == GPIO.HIGH:
                print("Button pressed!")
                button_pressed = True
        time.sleep(0.1)  # Check the button state every 100ms
...
try:
    while True:
        ...
        if current_user is None:
            ...
            # Check for a button press continuously after a user has badged in
            while current_user is not None:
                if button_pressed:
                    print(f"Detected button press by {current_user}")  # Added debug information
